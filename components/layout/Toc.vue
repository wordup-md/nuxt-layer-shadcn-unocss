<template>
  <UiScrollArea v-if="!isSmall" orientation="vertical" class="z-30 hidden overflow-y-auto md:block lg:block"
    type="hover">
      ...
      <div v-if="toc?.links.length">
        ...
        <div v-if="links" class="pt-5 text-muted-foreground">
          <NuxtLink v-for="(link, i) in links" :key="i" :to="link.to" :target="link.target"
            ...
            <SmartIcon v-if="link.icon" :name="link.icon" class="mr-1 self-center" />
            {{ link.title }}
            <Icon v-if="link.showLinkIcon ?? true" name="lucide:arrow-up-right"
              class="ml-auto self-center text-muted-foreground" size="13" />
          </NuxtLink>
        </div>
      </div>
      <!-- <div class="flex-grow" /> -->
